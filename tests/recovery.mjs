...
for(const kind of ['cat','vocal','dj']){
 ...
 for(let cycle=0;cycle<3;cycle++){
  ...
  b.release();for(let i=0;i<108;i++)b.step(1/90,settings);b.prepareRender();
  ...
  assert(peak<.0001);console.log(kind,'cycle',cycle,'visible shape residual',peak);
 }
}
...
// Short pulls and re-grabs must not reveal the hidden solver strain again.
for(const kind of ['cat','vocal','dj'])for(const point of [[.7,.58,-.6],[-.9,.58,.2],[1.4,.2,0]]){
 const map=shapeMapper(kind),b=new Jelly(map),skin=b.skin(point,map(point));
 for(let cycle=0;cycle<3;cycle++){
  b.prepareRender();const start=b.renderSample(skin,[0,0,0]);b.pin(skin,start);b.prepareRender();
  assert(Math.hypot(...b.renderSample(skin,[0,0,0]).map((v,k)=>v-start[k]))<1e-6,'re-grab pop');
  for(let i=0;i<36;i++){b.grab.target=start.map((v,k)=>v+(k===1?.22:k===0?.12:0)*Math.min(1,i/18));b.step(1/90,settings);}
  b.release();for(let i=0;i<108;i++)b.step(1/90,settings);b.prepareRender();
  const f=b.frame(),p=b.renderSample(skin,[0,0,0]);
  const rest=f.center.map((v,k)=>v+skin.surfacePoint.reduce((sum,v,j)=>sum+f.rotation[k*3+j]*(v-f.restCenter[j]),0));
  assert(Math.hypot(...p.map((v,k)=>v-rest[k]))<.0001,'short pull retained dent');
 }console.log('PASS short pull / re-grab',kind,point);
}
